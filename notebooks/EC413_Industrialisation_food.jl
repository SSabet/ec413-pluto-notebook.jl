...
md"# _Industrialisation and the Role of Food (Optional Model)_
...
md"""
## Does the Essentiality of Food Matter for Take-Off to the Modern Growth?
In this notebook we present a model in which the economic takeoff might or might not happen if food is a necessity.
...
The household consumes food (essential good) and industrial goods (not essential) and makes fertility decision.

The household preferences are represented with the utility function (dropping the $t$ subscripts):

$u(c_a, c_i, n) = \begin{cases}
       \log(c_{a}) + \log(n) &\quad\text{if } c_{a} < f \\
       \log(f+c_{i}) + \log(n) &\quad\text{if } c_{a} \geq f \\

     \end{cases}$
where $c_a$ and $c_i$ denote the consumption of the agricultural good (food) and the industrial good respectively, while $n$ denotes the fertility, that is the number of childrens per adult. $N_t$ is the population at time $t$, where each $t$ denotes a generation.

So the population grows with the law of motion:
$N_{t+1} = n_t N_t$.

- Interpretation: if the household is poor they just consume food; only when their demand for food gets fully satiated ($c_a \geq f$), they start consuming industrial goods.


"""

# ╔═╡ 662a33cc-bb8f-42a8-93c7-6f4c1ec21e03
md"""
### Production
There are two sectors the product of which are not substitutable for the household. The agricultural sector produces according to

$Y_{at} = \left(A_{at}X \right)^\alpha N_{at}^{1-\alpha},$

where $X$ is the quantity of land (in fixed supply).

The industrial sector has a production function which is linear in labour:

$Y_{it} = A_{it} N_{it}.$

Productivity in the two sectors grow with constant rates $g_a$ and $g_i$ respectively.

$A_{x,t+1} = (1+g_x) A_{xt}, \qquad x\in\{a,i\}.$
...
# ╔═╡ f431e1a1-1dfc-4c36-a8eb-47198de57ce5
md"""
...
# ╔═╡ c15877f2-0322-4e8e-b3cd-ac81ced777f3
md"""
Let's simulate the evolution of this economy for T=150 periods. Below is a function that to compute the transition path. It's hidden by default, click on the eye icon if you're interested in reading the code.
"""

# ╔═╡ be7835a6-3266-4038-a484-90307686fbf4
function transition(T,α,f, gₐ, gᵢ)
	...
	Aa = Aa[1]*(1+gₐ).^(0:T-1)
	Ai = Ai[1]*(1+gᵢ).^(0:T-1)
	#malthusians = BitArray(undef,T) # indicator of whether we are still at the Malthusian phase at time t
	...
	Malthusian = !(mₜ^(-α) > f) # indicator whether we are at the Malthusian phase now
	...
	n[t] = nₜ
	m[t] = mₜ
	#malthusians[t] = Malthusian

end
	return (Aa,Ai, Ya,Yi, Na, Ni, N, n,m)
end
...
begin
	Aa,Ai, Ya,Yi, Na, Ni, N, n,m = transition(T,α,f, gₐ, gᵢ)
	nothing
end
...
begin
	Y = Ya + Yi
	YN = Y./N
	YN_gr = YN[2:end]./YN[1:end-1]
	
	plot(1:T-1, YN[1:T-1], label = "Over time", lw = 2, xlabel = L"$t$", ylabel = L"$\frac{Y_t}{N_t}$", aspect_ratio = :none,fmt=:png)
	p_malthus1 = plot!(1:T-1, (1+gₐ)*ones(T-1), ls = :dash, lw=2, label = L"BGP")

	plot(1:T-1, n[1:T-1], label = "Over time", lw = 2, xlabel = L"$t$", ylabel = L"$n_t$", aspect_ratio = :none, legend=:topleft, fmt=:png)
	p_malthus2 = plot!(1:T-1, (1+gₐ)*ones(T-1), ls = :dash, lw=2, label = L"BGP")

	plot(1:T-1, Na[1:T-1]./N[1:T-1], label = "Over time", lw = 2, xlabel = L"$t$", ylabel = L"$\frac{N_a}{N}$", yaxis=[0,1], aspect_ratio = :none, legend=:bottomleft,fmt=:png)
	p_malthus3 = plot!(1:T-1, ones(T-1), ls = :dash, lw=2, label = "Malthusian BGP")

	m_ss = (1/(2p*(1+gₐ)))^(1/α)
	plot(1:T-1, m[1:T-1], label = "Over time", lw = 2, xlabel = L"$t$", ylabel = L"$m_t$", aspect_ratio = :none,legend=:bottomright, ylimits=(0,2),fmt=:png)
	p_malthus4 = plot!(1:T-1, m_ss*ones(T-1), ls = :dash, lw=2, label = "Malthusian BGP")

	plot(p_malthus1,p_malthus2, p_malthus3, p_malthus4, layout=(2,2))
end
